...
function resetToEmpty() {
  if (!confirm("지금 보고 있는 무드보드를 닫고 새로 검색할까요? (저장된 결과가 삭제되고, 완전히 새로 시작합니다)")) return;
  // 이전에 요청했던 자료가 남아있지 않도록 저장된 데이터까지 함께 지운다.
  if (storageKey) localStorage.removeItem(storageKey);
  localStorage.removeItem(LAST_KEY_POINTER);
  currentData = null;
  storageKey = null;
